...
/*
let arrResult5
// 이 줄이 let이고 우리가 나중에 이 값을 변경할 수 있어서 이 줄에서 값을 주지 않아도 괜찮은데 const arrResult5 적어두고 값을 바로 주지 않으면 아예 오류가 남. const 속성 자체가 절대로 값이 바뀌지 않는 애니까

if (arr1[5]) {      // 우리 arr1에 5번째 요소가 false이므로 이 줄을 false인 경우의 상태
    arrResult5 = `${arr1[4]} ${arr1[6]}`
} else {            // 반대로 여기는 true인 상탱
    arrResult5 = arr1[1] + arr1[3]
}
console.log(arrResult5)


if(0), if(2), if(4) 이런식으로 작성이 되면 
0이 아닌 모든 수를 true로 인식하기 때문에 
false, true, true의 값을 갖는다.
만약에 chaine을 사용한다고 하면, 
if('0'), if('fiejf') 같이 값이 들어간 애들은 true로 
생각하는데 if('')는 false 값을 갖는다. 
왜냐면 얘는 undefined이기 때문. undefined 와 null은
false의 값을 갖는다.
*/
